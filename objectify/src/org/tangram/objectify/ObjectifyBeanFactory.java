...
@Named("beanFactory")
@Singleton
public class ObjectifyBeanFactory extends AbstractMutableBeanFactory<Objectify, Query<?>> implements MutableBeanFactory<Objectify, Query<?>> {

    private static final Logger LOG = LoggerFactory.getLogger(ObjectifyBeanFactory.class);
    ...
    public <T extends Content> T getBean(Class<T> cls, String kind, String internalId) throws Exception {
        ...
        Long id = Long.parseLong(internalId);
        LoadResult<? extends Content> result = ObjectifyService.ofy().load().type(kindClass).id(id);
        Content now = result.now();
        LOG.debug("getBeans() {}:{} {}", kindClass.getSimpleName(), id, now);
        return convert(cls, now);
    } // getBean()


    @Override
    public ObjectifyContent getBean(String id) {
        return getBean(ObjectifyContent.class, id);
    } // getBean()


    @Override
    public Class<? extends Content> getBaseClass() {
        return ObjectifyContent.class;
    } // getBaseClass()
    ...
    public Query<?> createQuery(Class<? extends Content> cls, String expression) {
        Query<? extends Content> query = ObjectifyService.ofy().load().type(cls);
        if (StringUtils.isNotEmpty(expression)) {
            // TODO: Use query expression
            LOG.warn("createQuery() query expression ignored");
        } // if
        return query;
    } // createQuery()
    ...
    public <T extends Content> List<T> listBeans(Class<T> c, String query, String orderProperty, Boolean ascending) {
        ...
        try {
            for (Class<T> cls : getImplementingClasses(c)) {
                ...
                Query<T> q = ObjectifyService.ofy().load().type(cls);
                if (StringUtils.isNotEmpty(query)) {
                    // TODO: use where clause
                    LOG.warn("listBeans() query string ignored");
                } // if
                ...
                LOG.info("listBeans() objectify query object is {} ", q);
                for (T t : q) {
                    results.add(t);
                } // for
    ...
    public <T extends Content> List<T> listBeans(Query<?> query) {
        ...
        try {
            LOG.info("listBeans() objectify query object is {} ", query);
            for (Object t : query) {
                result.add(SystemUtils.convert(t));
            }
            ...
            if (allClasses==null) {
                ...
                try {
                    ...
                    if (classNames==null) {
                        ...
                        resolvedClasses.addAll(resolver.getAnnotatedSubclasses(ObjectifyContent.class, Entity.class));
